...
class db_users:
    ...
    def get_user_by_id(self, id):
        ...
        if user is None:
            ...
            return User.User(user[0], user[1], user[2], user[3], user[4], user[5], pickle.loads(user[6]), pickle.loads(user[7]))
    ...
    def query_to_user(self, query):
        ...
        user_list = []
        for user in users:
            user_list.append(User.User(user[0], user[1], user[2], user[3], user[4], user[5], pickle.loads(user[6]), pickle.loads(user[7])))
        return user_list
    ...
    def result_to_user(self, user):
        ...
        """
        return User.User(user[0], user[1], user[2], user[3], user[4], user[5], pickle.loads(user[6]) if user[6] is not None else None, pickle.loads(user[7]) if user[7] is not None else None)
        ...
        if reservations is not None:
            ...
            for i in range(len(reservations)):
                ...
                if reservations[i][0] == house_id:
                    reservations.pop(i)
                    break
            self.cur.execute("UPDATE users SET reservations = ? WHERE id = " + str(user_id), (pickle.dumps(reservations),))
            self.conn.commit()
            return True
        else:
            return False
...
# make user
u = db_users()
#userr = User.User(25823734, "d", "p", 1, "e", 17)
#u.add_user(userr)
u.get_all_users()
#print(u.login("d", "p"))
"""
